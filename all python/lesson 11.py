...
def do_twice(func_add, x, y): # принимает на вход функцию и два параметра, функцию принимает для дальнейшего использования
...
def do_twice(func_add, x, y): # принимает на вход функцию и два параметра
    return func_add(func_add(x, y), func_add(x, y)) # В скобках выполняется 2 раза, а результаты берутся как новые аргументы
